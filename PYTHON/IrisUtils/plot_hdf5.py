    ...
    if reciprocal_calib:
        ...
        if ul_data_avail > 0:
            ...
            if demodulate:
                ...
                print('uplink_data:', ul_data.shape)
                # print(ul_data)

                # print("=======   start saving the uplink data  =======")
                ...
                # np.save('{}uplink_data.npy'.format(path), ul_data)
                # print("=======   finish saving the uplink data   =======")
                ...
                # print('initialize txdata:', txdata)
                print('initialized txdata.shape:', txdata.shape)
                if ul_data_frame_num > 1:
                    ...
                    for f in range(ul_data_frame_num):
                        print('ul_data_frame_num:', ul_data_frame_num)
                        for i in range(num_cl_tmp):
                            tx_data0 = metadata['OFDM_DATA_CL'+str(i)]
                            print('tx_data0:', tx_data0.shape)
                            tx_data_mat = np.reshape(tx_data0, (ul_slot_num, symbol_per_slot, fft_size))
                            print('tx_data_mat:', tx_data_mat.shape)
                            txdata[f, i, :, :, :]  = tx_data_mat
                            print('txdata.shape:', txdata.shape)
                    txdata_ext = np.tile(txdata, (n_frames, 1, 1, 1, 1))
                txdata_ext = txdata_ext[:, :, :, :, data_sc_ind]

                # exclude the zero and pilot sub-carriers

                # print('txdata:', txdata)
                print('txdata.shape:', txdata.shape)
                # print('txdata_ext:', txdata_ext)
                print('txdata_ext.shape:', txdata_ext.shape)
                ...
                for i in range(num_cl_tmp//2):
                    ...
                    # print('ref_frame:', ul_data[ref_frame, i * 2, ul_sym_i, :].shape)
                    # print('tx_data:', txdata_ext[ref_frame, i * 2, ul_sf_i, ofdm_sym_i, :])
                    ...
                    # print('real{}:'.format(i), np.real(ul_data[ref_frame, i * 2, ul_sym_i, :]))
                    # print('imag{}:'.format(i), np.imag(ul_data[ref_frame, i * 2, ul_sym_i, :]))
                    #
                    # print('real{}:'.format(i), np.real(txdata_ext[ref_frame, i * 2, ul_sf_i, ofdm_sym_i, :]))
                    # print('imag:{}'.format(i), np.imag(txdata_ext[ref_frame, i * 2, ul_sf_i, ofdm_sym_i, :]))
...
def main():
    ...
    parser.add_option("--n-frames", type="int", dest="n_frames", help="Number of frames to inspect", default=4000)
