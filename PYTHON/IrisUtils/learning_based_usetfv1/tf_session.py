...
class MMNet_graph():
    ...
    def build(self):
        ...
        with tf.device('/gpu:0'):
            ...
            # Load in transmit signals
            constellation = mimo.constellation
            indices = mimo.random_indices()
            data = np.load('{}uplink_data.npy'.format(data_path))
            x = data[:,:,:,0]
            ...
            print('x.shape:', x.shape)
            x = complex_to_real(x)
            print('x.shape after complex to real:', x.shape)
            # x = x.reshape(-1, 2*self.params['K'])
            # print('x.shape:', x.shape)
            x = tf.Variable(x, trainable=False, dtype=tf.float32)

            # Send x through the channel
            if self.params['data']:
                # params['N'] refers to receive antennas, params['K'] refers to transmit antennas
